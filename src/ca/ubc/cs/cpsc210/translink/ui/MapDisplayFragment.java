...
public class MapDisplayFragment extends Fragment implements MapEventsReceiver, IMyLocationConsumer {
    ...
    /** stop that is nearest to user (null if no such stop) */
        ...
        for (Route r : RouteManager.getInstance()) {
            if(r.hasStop(s)) {
                for (RoutePattern rp : r.getPatterns()) {
                    ...
                    for (int i = 0; i < rp.getPath().size(); i++) {
                        LatLon latlon = new LatLon(rp.getPath().get(i).getLatitude(), rp.getPath().get(i).getLongitude());
                        lat = latlon.getLatitude();
                        lon = latlon.getLongitude();
                        gps.add(new GeoPoint(lat, lon));
                    }
...
//                + southeast.getLatitude() + " " + southeast.getLongitude());
    ...
    private void markStops() {
        ...
        for(Stop s: StopManager.getInstance()) {
            /** only mark stops that are in the visible part of the map
             * does not create a new marker if the visible stop is the nearest stop
             */
            if(Geometry.rectangleContainsPoint(northWest,southEast,s.getLocn()) && s != nearestStop) {
                ...
                String info = Integer.toString(s.getNumber()) + " " + s.getName();
                info += "\n";
        ...
        //adds the markers of all visible stops, including the nearest stop marker, to the map
        for(Stop s: StopManager.getInstance()) {
    ...
    private void updateMarkerOfNearest(Stop nearest) {
        ...
        //if nearest stop is null then nearestStopMarker and nearestStop are both null
        if(nearest == null) {
            nearestStopMarker = null;
            nearestStop = null;
        }
            ...
            for(Stop s: StopManager.getInstance()) {
                if(nearest.equals(s)) {
                    //if nearest stop doesn't already have a marker create one
                    if(getMarker(s) == null) {
                        double lon = s.getLocn().getLongitude();
                        double lat = s.getLocn().getLatitude();
                        GeoPoint geo = new GeoPoint(lat, lon);
                        ...
                        marker.setRelatedObject(s);
                        marker.setInfoWindow(stopInfoWindow);
                        marker.setTitle(Integer.toString(s.getNumber()) + " " + s.getName());
                        marker.setIcon(closestStopIconDrawable);
                        setMarker(s, marker);
                        nearestStopMarker = getMarker(s);
                        nearestStop = s;
                    //if nearest stop already has a marker set the icon to closestStopIconDrawable
                    } else {
                    ...
                    //if the stop is not nearest and stop does not have a marker then create one and set the icon to stopIconDrawable
                    if(getMarker(s) == null) {
                        double lon = s.getLocn().getLongitude();
                        double lat = s.getLocn().getLatitude();
                        GeoPoint geo = new GeoPoint(lat, lon);
                        ...
                        marker.setIcon(stopIconDrawable);
                        setMarker(s, marker);
                    //if the stop is not nearest and already has a marker then set the icon to stopIconDrawable
                    } else {
    ...
    private void handleLocationChange(Location location) {
        Stop s = StopManager.getInstance().findNearestTo(new LatLon(location.getLatitude(), location.getLongitude()));
        locationListener.onLocationChanged(s, new LatLon(location.getLatitude(),location.getLongitude()));
