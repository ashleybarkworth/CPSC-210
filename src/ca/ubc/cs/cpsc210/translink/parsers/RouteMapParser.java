...
public class RouteMapParser {
    ...
    private void parseOnePattern(String str) {
        if (str.length() != 0) {
            ...
            String number = parts1[0].substring(1);
            List<LatLon> list = new ArrayList<>();
            int index = str.indexOf('-');
            String[] parts2 = str.substring(index + 1).split(";");
            String name = parts2[0];
            if(parts2.length > 1) {
                for (int i = 2; i < parts2.length; i+=2) {
                    LatLon newLocation = new LatLon(Double.parseDouble(parts2[i-1]), Double.parseDouble(parts2[i]));
                    list.add(newLocation);
                }
            }
            storeRouteMap(number, name, list);
        }
    }
